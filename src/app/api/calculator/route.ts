...
export async function POST(request: NextRequest) {
  ...
  const accessCheck = await withRoleCheck(AccessRequirements.AUTHENTICATED)(request, async () => {
    try {
      const { results: tariffs } = await db.prepare(
        `SELECT * FROM tariffs WHERE id = ?`
      ).bind(data.tariff_id).all();
...
const initialPaymentAmount = data.property_price * (tariff.initial_payment_percent / 100);
const monthlyPaymentAmount = data.property_price * (tariff.monthly_payment_percent / 100);
...
export async function GET(request: NextRequest) {
  const accessCheck = await withRoleCheck(AccessRequirements.MEMBER_OWN_DATA)(request, async () => {
    try {
      ...
      const { results: calculations } = await db.prepare(
